...
def prepare_dataset(path, name):
    ...
    logger.info(f"{len(entities)} entities and {len(relations)} relations found.")
    n_relations = len(relations)
    n_entities = len(entities)
    os.makedirs(os.path.join(DATA_PATH, name), exist_ok=True)

    # Check if the pickle files already exist
    if not os.path.exists(os.path.join(DATA_PATH, name, 'train.pickle')):
        ...
        for f in files:
            ...
            out.close()
            logger.info(f"Processed {f} dataset with size: {len(examples)} entries.")

    logger.info("Creating filtering lists")
...
if __name__ == "__main__":
    ...
    for d in datasets:
        logger.info(f"Preparing dataset {d}")
        try:
            prepare_dataset(
                os.path.join(
            ...
            if e.errno == errno.EEXIST:
                logger.warning(f"File exists. Skipping dataset preparation for {d}.")
            else:
                raise
